...
ggplot(data = df2, mapping = aes(x = TIME, y = INT, group = ID, color = ID)) +
  geom_point() +
  geom_line(mapping = aes(y = predicted), alpha = 0.25) #, linetype = "dotted")
```
...
```{r}
ggplot(data = df, mapping = aes(x = TIME, y = INT, color = STATUS)) +
...
model.fn <- function(psi,id,x) { 
  ...
  plateau = psi[id,1]
  rate = psi[id,2]
  fpred = plateau * (1 - exp(-rate * t))
  return(fpred)
}

saemix.model <- saemixModel(model = model.fn, 
                            psi0  = c(plateau = 10000000, rate = 0.5))
```
